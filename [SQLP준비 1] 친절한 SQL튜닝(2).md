- 본 글은 개인 공부 내용을 요약한 글이며 책의 상당 부분이 빠져있음
...
- CBO (Cost Based Optimizer)
  - 데이터 딕셔너리의 다양한 통계정보를 이용해 예상비용과 실행계획 산출
    - 데이터량, 컬럼 값의 수·분포, 인덱스 높이, 클러스터링 팩터


과거에는 RBO(rule based Optimizer) 사용
- RBO : 정해진 규칙에만 의존하여 실행계획 결정
  - 규칙에만 의존하기 때문에 현재같은 대용량 데이터베이스 체제에서는 효과적이지 않음
  - ex) 고객유형코드 에 대한 `=`조건 검색에 인덱스 사용.
  이 때 한 고객 유형이 전체의 90% 차지하여 인덱스 효율이 낮은데도 규칙 우선순위에 따라서 인덱스 스캔 사용함
...
- FIRST_ROWS 모드일 경우 풀 스캔보단 인덱스 스캔
해시·소트 머지조인 보단 NL조인을 선택하는 경향이 생김
...
4. 통계 정보
   - 아래와 같은 사항들은 시스템 장애를 유발할 수 있음
     - 특정 테이블 통계정보가 갑자기 삭제됨
     - 대량 데이터 삭제 후 재입력 전 빈 상태에서 자동으로 통계정보 수집됨
     - 장기간 갱신되지 않은 테이블의 통계정보 재수집
     - 통계정보가 없던 테이블에 인덱스 재생성 (인덱스 생성 시 통계정보가 자동으로 수집됨)
     - 테이블/인덱스 재생성 시 파티션 단위로만 통계정보 수집
       - 바인드 변수 사용 시 파티션 통계가 아닌 Global통계가 사용됨
...
6. 옵티마이저 관련 파라미터
   - 일반적으로 버전이 바뀌는 경우 옵티마이저의 기본값이 변경되거나 파라미터가 추가되어 기존과 다르게 동작할 수 있음
...
- 같은 SQL이라도 DBMS, 버전, 통계정보 등에 따라 다르게 동작할 수 있음
- 통계정보를 항상 업데이트 된 상태로 둘 수도, 그 정보가 필요한 정보의 전부일 수도 없음


- 바인드 변수가 활용된 SQL에는 컬럼 히스토그램을 활용할 수 없음


등 다양한 이유로 옵티마이저는 분명한 한계를 가짐
...
- 최적의 옵티마이징 팩터 제공
  - 전략적인 인덱스 구성
  - DBMS의 다양한 기능 사용
    - 파티션, 클러스터, IOT, MV, Result cache 등
  - 옵티마이저 모드 설정
  - 정확하고 안정적인 통계정보
...
- autotrace의 실행계획과 실행통계를 확인하기 위해서는 몇몇 뷰에 대한 읽기 권한이 필요하다
...
- tkprof 유틸리티를 사용해 트레이스 파일을 리포트로 보기좋게 생성해줌

## 부록-4. DBMS_XPLAN 패키지

- 라이브러리 캐시에 캐싱된 SQL 실행계획 확인 가능
- SQL트레이스처럼 오퍼레이션 단계별 수행통계도 확인 가능
...
-- 생성한 실행계획 분석
select * from table(
	dbms_xplan.display('PLAN_TABLE', 'SQL1', 'BASIC ROWS BYTES COST')
);
```
`BASIC` 외에도 `TYPICAL`, `SERIAL`, `PARTITION` 등 다양한 항목 있음


#### 캐싱된 커서의 실제 실행계획 출력
- SQL 커서 : 하드파싱 후 메모리에 적재된 SQL, 파싱트리, 실행계획, SQL 실행에 필요한 정보를 담은 'SQL Area' 
  - 라이브러리 캐시에 캐싱됨, 캐싱된 커서에 대한 수행통계를 v$sql 뷰를 통해 조회할 수 있다.
...
- 실시간 모니터링 대상
  - CPU time 혹은 IO time을 5초 이상 소비한 SQL
  - 병렬SQL
  - monitor 힌트를 지정한 SQL

→ 실행 계획이 500라인 초과 시 모니털이 대상에서 제외


## 부록-6 V$SQL

- 라이브러리 캐시에 캐싱된 SQL들에 대한 수행통계 제공

551p~552p의 스키마별 통계 수집 쿼리 확인하기
