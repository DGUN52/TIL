...
- 첫번째 기준 : 조건절에 항상 사용하거나, 자우 사용하는 컬럼을 선정
...
a를 스캐하는 과정에서의 비효율은 감당할만하다. 단 b쪽에 비효율적인 인덱스 전략이 있다면 a에서 b를 액세스 하는 만큼 비효율이 발생하니 치명적
...
이 때 액세스 조건이 필터 조건이 되어버리는 경우는 효율성이 떨어짐(`=`조건이 범위검색 조건이 될 수도 있는경우)
...
(계약에 조건을 걸어서 조회되는 양이 낮다면 인덱스의 두번째 컬럼이 사실상 무의미하기 때문)
