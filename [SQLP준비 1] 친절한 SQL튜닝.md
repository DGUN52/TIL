...
- 인덱스 탐색 : 리프 블록 탐색 → 리프블록의 ROWID → 해당 블록 접근
...
- 인덱스를 사용할 대상 레코드가 아무리 많아도 빠른 응답속도를 낼 수 있는 방법 : 부분범위 처리

### 3.2.1 부분범위 처리

- Fetch Call이 있을 때마다 일정량씩 나누어 전송하는 것
- Array Size(클라이언트에서 결정)가 10인 상태에서 1건씩 호출할 때, Array Size인 10건씩 클라이언트의 캐시에 적재한다.
  - 적재한 데이터를 다 사용하면 추가 Fetch Call을 통해 10건을 더 받고, 이를 반복한다.


- 데이터를 어차피 끝까지 전송해야 한다면 Array Size를 늘려서 Fetch Call을 줄이는 것이 바람직
- 반대로 데이터를 읽다가 그만두는 경우가 많다면 Array Size를 줄여서 버려질 데이터의 낭비를 줄이는 것이 바람직


- 이런 부분범위 처리는 어느 DBMS나 수행하지만, Fetch Call 을 사용자의 요청에 따라 처리할지, 자동으로 계속 불러올지는 프로그램마다 다르다. 

### 3.2.2 부분범위 처리 구현

### 3.2.3 OLTP환경에서 부분범위 처리에 의한 성능개선 원리

- 은행 입출금, 게시판, 뉴스 등의 경우 상위 일부 데이터만 처리(OLTP)하기때문에
해당하는 컬럼에 인덱스를 걸어서 항상 정렬상태를 유지하게하여, 부분범위 처리를 가능하게 하면 응답속도가 빠르다.
...
```
위 예시에서 인덱스는 (code, regit_date) 로 이루어져야 소트연산 생략이 가능하고, 부분범위 처리가 가능하다.


- 배치IO : 버퍼 캐시에서 블록을 찾지 못하면 발생하는 DISK IO를 미뤘다가, 일정량 쌓이면 한꺼번에 처리
- 배치IO가 작동하면 정렬순서가 보장되지 않는다. 따라서 sort order를 생략할 수 없다. (부분범위 처리 불가)
...
- 인덱스 액세스 조건 : 수직 탐색 조건
- 인덱스 필터 조건 : 테이블 액세스 조건
- 테이블 필터 조건 : 테이블 액세스 후 필터링하는 조건


- 테이블 액세스 비용 = 인덱스 수직 탐색 + 인덱스 수평 탐색 + 테이블 랜덤 액세스 비용


### 3.3.4 비교 연산자 종류와 컬럼 순서에 따른 군집성

- 선행 컬럼이 모두 `=` 조건일 때, 이후 등장하는 첫번째 범위검색까진 모여있다. 
(첫번째 등장하는 범위검색까진 인덱스 액세스 조건으로 사용된다.)
...
- between, 부등호 조건 대신 IN으로 풀어사용하면 INDEX를 더 잘 활용할 수 있다.
- IN-List 항목이 늘어날 수 있다면 NL방식의 조인문 / 서브쿼리로 구현하면 된다. 
(IN-List의 항목들을 코드 테이블로 관리할 때 가능)


- IN-List의 항목이 많고 브랜치의 깊이가 깊으면 비효율적이게 될 수있음 
(IN-List 항목만큼탐색 횟수가 늘어나기때문)
```sql
where GRADE between 'C' and 'D'
and EMPNO = 123
```
Between조건을 IN-List로 전활할 때,
위의 예시 같은 경우 (C, 123)과 (D, 123)이 거리가 효율이 극대화된다.

> 250211(화) 159p ~ 198p

### 3.3.7 Index Skip Scan 활용

- 멀리 떨어진(전체 중 비중이 낮은) 인덱스 컬럼에 대해 IN-List 전환과 Index Skip Scan은 좋은 효율을 가진다.
...
ex) 
```sql
t_idx01(고객번호, 상품번호) > t_idx02(상품번호, 고객번호)
select -
from t
where 고객번호 = 12345 and 상품번호 IN (A1, B92, C901)
```
→ 상품은 일반적으로 수많은 고객번호에 의해 구매되고, 고객은 평균적으로 3개의 상품을 구매한다.(카디널리티 3)

상품번호가 선두 인덱스 컬럼이라면 `A1`, `B92`, `C901`에 대해 IN-List 형식으로 풀려야하고,
흩어진 고객번호를 찾기 위해 3번의 수직탐색 후에 데이터를 찾는다.

반면 고객번호가 인덱스 선행 컬럼이라면 상품번호는 정렬된 상태이기 때문에 하나의 리프블록에 저장돼있을 확률이 높다. (또는 연속된 두개의 리프블록)
따라서 1번의 수직탐색 (+1번의 인접한 리프블록 탐색)으로 결과를 얻을 수 있다.

- **`IN` 조건은 `=`조건이 아니다.**


- `select /*+ num_index_keys(table_a table_idx01 1) */`
  - 첫번째 인덱스 컬럼 까지만 액세스 조건으로 활용하라는 힌트
  
### 3.3.9 BETWEEN과 LIKE 스캔 범위 비교

- 문자열에 LIKE 를 쓰는 것보다 BETWEEN을 쓰는 것이 효율적이다.
- LIKE에 %연산자를 활용하면 스캔 시작지점과 종료지점을 찾을 때 BETWEEN보다 추가적인 탐색을 한다. 
(String에는 위아래 끝이 없기 때문에 가능성을 계속 찾아야된다.)
(ex `BETWEEN 201901 AND 201912` vs `LIKE '2019%'` 일 때 LIKE는 201901 전에도 더 읽고, 201912 뒤에도 더 찾는다.)
...
조건이 있을 수도 없을 수도 있는 경우 쿼리를 2개를 써야한다.
...
AND 상품 LIKE :prod || '%'
 ...
 → 지역코드가 있을 수도 없을 수도 있다.
```

위처럼 나뉘어야 하는데 `지역`을 그냥 LIKE로 처리해버리는 경우
```sql
SELECT ~
FROM TBL A
WHERE 회사 = :com
...
이런 경우는 개발에만 편리하고, `지역`이 입력됐을 때 쿼리의 효율은 떨어진다.

비슷한 예로 문자열을 있거나 없는 경우를 나누지 않고 없을 경우 `BETWEEN '____' AND 'ZZZZ'` 로 하는 경우도 있다.
...
#### or조건
```sql
where :cust_id is null or 고객_id = :cust_id
```
→ or expansion 불가 (좌측의 조건은 컬럼조건이 아니기 때문)

- OR조건으로 사용된 컬럼은 인덱스로 활용이 불가능할 수 있다.
- OR로 결합된 모든 조건문이 인덱스를 사용가능한지, not null 제약이 있는지 확실히 해야한다.

#### LIKE/BETWEEN조건

```sql
where 상품코드 like :prcd_cd || '%'
```
이런식으로 옵션절+`%`를 사용하는 것은 비효율을 부른다.

- LIKE/BETWEEN 사용 시 마이너스 요소
  1. 인덱스 선두컬럼인지
  2. NULL 허용컬럼인지 → Null 비교문이 되면 결과집합에서 사라진다.
  3. 숫자형 컬럼인지
  4. 가변 길이 컬럼인지 - 원하지 않는 출력을 제거하기 위해 조건을 추가하여 사용한다.
  ex)
  ```sql
...
AND 거래일자 BETWEEN :dt1 and :dt2
```
→ UNION ALL 위에선 (거래일자) 인덱스를 사용하고 아래에선 (고객ID, 거래일자) 인덱스를 사용함. 비효율 없음
...
```
위 쿼리는 or expansion이 일어나서 효율적으로 실행계획을 도출한다
UNION ALL보다 단순하면서 UNION ALL의 성능을 낼 수 있음

- COALESCE, CASE문은 OR EXPANSION 불가
- NULL 허용 컬럼 불가
- 여러 번 사용 시 가장 변별력이 좋은 컬럼 기준으로 한번만 OR-EXPANSION 발생
...
#### PL/SQL함수의 성능적 특성
- PL/SQL 함수는 느리다.
- PL/SQL 함수는 호출횟수만큼 수행된다.
  - 회원이 100만건인 회원테이블을 FULL SCAN하면 100만건 실행되고, INDEX SCAN으로 스캔 수를 줄이면 그만큼만 실행된다.
  - 따라서 효과적인 인덱스 액세스 조건을 구성해야 PL/SQL함수로 인한 부하를 줄일 수 있다.
...
- 첫번째 기준 : 조건절에 항상 사용하거나, 자주 사용하는 컬럼을 선정
- 두번째 기준 : 그 중 `=`조건으로 사용되는 컬럼을 두어야 한다.
...
- NL조인에서 `/*+ leading(a) use_nl(b) */` 를 수행한다 할 때
a를 스캔하는 과정에서의 비효율은 감당할만하다. 단 b쪽에 비효율적인 인덱스 전략이 있다면 a에서 b를 액세스 하는 만큼 비효율이 발생하니 치명적


### 3.4.4 공식을 초월한 전략적 설계

- 공식대로 최적의 인덱스를 전부 만들 순 없다.(공간 제약, DML 성능 제약)
- 실제 부하에 대한 이해를 통해 인덱스 수를 줄여야 현재 DML의 부하도 줄이고, 후에 추가/변경된 업무 프로세스에 대해 인덱스를 추가할 수 있다.
...
- 컬럼을 추가하는 기준
1. `=`연산자로 사용된 컬럼
2. `ORDER BY` 절에 사용된 컬럼
3. `=`연산자가 아닌 조건절 컬럼은 데이터 분포에 따라 추가
추가로 인덱스 컬럼을 추가하는 것은 상관 없지만, 액세스 감소 효과가 큰 경우가 아니라면 DML부하를 생각해서 추가하지 않는 편이 좋다.
...
order by 연령
```
거주지역, 혈액형, 연령에 적절히 인덱스를 구성하여 sort연산을 생략할 수 있어보이지만,
IN-List Iterator 방식으로 쿼리가 풀리면 (UNION ALL로 'A', 'O'에 대해 각각 수행된 후 합친다.) 연령별로 정렬이 되지 않은 상태라 sort 옵션을 생략할 수 없다.

- 소트연산을 생략하려면 IN-List 방식으로 풀리면 안되고, 따라서 인덱스는 (거주지역, 연령, 혈액형) 으로 구성돼야한다.
- 혹은 `order by 혈액형, 연령`으로 작성되어야한다.


### 3.4.6 결합 인덱스 선택도
- 선택도 : 조건절에 의해서 선택되는 비율
- 카디널리티 : 선택도 * 총 레코드 수, 조건절에 속하는 원소의 개수

선택도, 카디널리티가 높다면 인덱스로서의 가치가 떨어진다.

단, 같은 `인덱스 액세스 조건`이라면 카디널리티에 따라서 인덱스 순서를 바꿀 필요는 없다. 순서를 바꾸어도 액세스 수는 똑같기 때문

`인덱스 필터 조건`으로 사용되는 컬럼끼리의 인덱스 순서도 바꾸는 것이 의미가 없다. 심지어 조건절의 연산자가 바뀌어도 상관이 없다.(어차피 스캔범위는 같음)

단 연산자가 바뀔 때 액세스 조건이 필터 조건이 되어버리는 경우는 효율성이 떨어짐(`=`조건이 범위검색 조건이 될 수도 있는경우)
...
여기서 계약ID의 카디널리티가 낮다면 사실상 중복인덱스 들이다.
(계약에 조건을 걸어서 조회되는 양이 적다면 인덱스의 두번째 컬럼이 사실상 무의미하기 때문)
이럴땐 후열의 컬럼들을 합쳐서 하나로 만든다.
...
- 거래일자, 결제일자는 항상 BETWEEN 혹은 부등호 조건으로 조회됨
- 데이터 분포는 아래와 같음
  - **컬럼명 NDV**
  거래일자 2356
  관리지점번호 127
